...
echo "=============================================="
echo "mCREAM: GraphMLP (M=10, Low) - CELEBA"
echo "=============================================="
echo "HOST=$(hostname)"
"$PYTHON_BIN" -V
nvidia-smi || true
...
EXPERT_DIR="$PROJECT_ROOT/data/CelebA/expert_graphs/M10/low"
if [ ! -d "$EXPERT_DIR" ]; then
    echo "Generating expert graphs (M=10, low)..."
    "$PYTHON_BIN" -c "
...
save_expert_graphs(expert_u2c, expert_c2y, '$EXPERT_DIR', {'disagreement_level': 'low', 'num_experts': 10})
print('Expert graphs saved!')
"
fi

echo ""
echo "Running mCREAM with GraphMLP aggregation (M=10, low)..."
"$PYTHON_BIN" mcream_main.py --config all_configs/mcream_configs/celeba/graph_mlp/graph_mlp_M10_low.yaml

...
echo "=============================================="
echo "mCREAM: GraphMLP (M=10, Medium) - CELEBA"
echo "=============================================="
echo "HOST=$(hostname)"
"$PYTHON_BIN" -V
nvidia-smi || true
...
EXPERT_DIR="$PROJECT_ROOT/data/CelebA/expert_graphs/M10/medium"
if [ ! -d "$EXPERT_DIR" ]; then
    echo "Generating expert graphs (M=10, medium)..."
    "$PYTHON_BIN" -c "
...
echo ""
echo "Running mCREAM with GraphMLP aggregation (M=10, medium)..."
"$PYTHON_BIN" mcream_main.py --config all_configs/mcream_configs/celeba/graph_mlp/graph_mlp_M10_medium.yaml

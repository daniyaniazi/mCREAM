...
echo "=============================================="
echo "mCREAM: GraphMLP (M=10, Structured Bias) - CELEBA"
echo "=============================================="
echo "HOST=$(hostname)"
"$PYTHON_BIN" -V
nvidia-smi || true
...
EXPERT_DIR="$PROJECT_ROOT/data/CelebA/expert_graphs/M10/structured_bias"
if [ ! -d "$EXPERT_DIR" ]; then
    echo "Generating expert graphs (M=10, structured_bias)..."
    "$PYTHON_BIN" -c "
from src.expert_graphs.generation import generate_expert_graphs_from_dag, save_expert_graphs, DISAGREEMENT_LEVELS
params = DISAGREEMENT_LEVELS['structured_bias']
expert_u2c, expert_c2y, u2c_star, c2y_star = generate_expert_graphs_from_dag(
    dag_path='data/CelebA/final_DAG_unfair.csv',
    num_classes=1, num_experts=10,
    p_del=params['p_del'], p_add=params['p_add'], p_rev=params['p_rev'],
    base_seed=42
)
save_expert_graphs(expert_u2c, expert_c2y, '$EXPERT_DIR', {'disagreement_level': 'structured_bias', 'num_experts': 10})
print('Expert graphs saved!')
"
fi

echo ""
echo "Running mCREAM with GraphMLP aggregation (M=10, structured_bias)..."
"$PYTHON_BIN" mcream_main.py --config all_configs/mcream_configs/celeba/graph_mlp/graph_mlp_M10_structured_bias.yaml

...
echo "=============================================="
echo "mCREAM: GraphMLP (M=5, Low) - CFMNIST"
echo "=============================================="
echo "HOST=$(hostname)"
"$PYTHON_BIN" -V
nvidia-smi || true
...
if [ ! -d "$EXPERT_DIR" ]; then
    echo "Generating expert graphs (M=5, low)..."
    "$PYTHON_BIN" -c "
...
echo ""
echo "Running mCREAM with GraphMLP aggregation (M=5, low)..."
"$PYTHON_BIN" mcream_main.py --config all_configs/mcream_configs/cfmnist/graph_mlp/graph_mlp_M5_low.yaml

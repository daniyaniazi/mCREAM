...
echo "=============================================="
echo "mCREAM: GraphMLP (M=5, High) - CFMNIST"
echo "=============================================="
echo "HOST=$(hostname)"
"$PYTHON_BIN" -V
nvidia-smi || true
...
EXPERT_DIR="$PROJECT_ROOT/data/FashionMNIST/expert_graphs/M5/high"
if [ ! -d "$EXPERT_DIR" ]; then
    echo "Generating expert graphs (M=5, high)..."
    "$PYTHON_BIN" -c "
...
save_expert_graphs(expert_u2c, expert_c2y, '$EXPERT_DIR', {'disagreement_level': 'high', 'num_experts': 5})
print('Expert graphs saved!')
"
fi

echo ""
echo "Running mCREAM with GraphMLP aggregation (M=5, high)..."
"$PYTHON_BIN" mcream_main.py --config all_configs/mcream_configs/cfmnist/graph_mlp/graph_mlp_M5_high.yaml

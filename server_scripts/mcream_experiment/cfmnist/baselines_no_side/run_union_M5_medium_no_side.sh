...
echo "=============================================="
echo "mCREAM: Union Baseline (M=5, Medium) - cfmnist - NO SIDE CHANNEL"
echo "=============================================="
echo "HOST=$(hostname)"
"$PYTHON_BIN" -V
nvidia-smi || true
...
if [ ! -d "$EXPERT_DIR" ]; then
    ...
    echo "Run generate_expert_graphs first!"
    exit 1
fi

echo ""
echo "Running mCREAM with Union aggregation (no side channel)..."
"$PYTHON_BIN" mcream_main.py --config all_configs/mcream_configs/cfmnist/baselines_no_side/union_M5_medium_no_side.yaml

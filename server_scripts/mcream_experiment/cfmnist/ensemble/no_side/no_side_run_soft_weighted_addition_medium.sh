...
echo "================================================"
echo "mCREAM Ensemble: cfmnist | soft_weighted | addition | medium"
echo "================================================"
echo "HOST=$(hostname)"
"$PYTHON_BIN" -V
nvidia-smi || true
...
EXPERT_DIR="$PROJECT_ROOT/data/FashionMNIST/expert_graphs/ensemble/addition_medium"
if [ ! -f "$EXPERT_DIR/config.yaml" ]; then
    echo "Generating expert graphs for addition/medium..."
    "$PYTHON_BIN" generate_ensemble_expert_graphs.py --dataset cfmnist --num_experts 5
fi

echo ""
echo "Running mCREAM Ensemble (soft_weighted, addition, medium)..."
"$PYTHON_BIN" mcream_ensemble_main.py     --config all_configs/mcream_ensemble_configs/cfmnist/no_side/no_side_soft_weighted_addition_medium.yaml

...
echo "================================================"
echo "mCREAM Ensemble: cfmnist | soft_weighted | reversal | low"
echo "================================================"
echo "HOST=$(hostname)"
"$PYTHON_BIN" -V
nvidia-smi || true
...
EXPERT_DIR="$PROJECT_ROOT/data/FashionMNIST/expert_graphs/ensemble/reversal_low"
if [ ! -f "$EXPERT_DIR/config.yaml" ]; then
    echo "Generating expert graphs for reversal/low..."
    "$PYTHON_BIN" generate_ensemble_expert_graphs.py --dataset cfmnist --num_experts 5
fi

echo ""
echo "Running mCREAM Ensemble (soft_weighted, reversal, low)..."
"$PYTHON_BIN" mcream_ensemble_main.py     --config all_configs/mcream_ensemble_configs/cfmnist/no_side/no_side_soft_weighted_reversal_low.yaml

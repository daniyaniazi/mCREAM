...
DAG_CSV="$PROJECT_ROOT/data/FashionMNIST/expert_graphs/ensemble/deletion_low/cream_noisy_dags/expert_3.csv"
if [ ! -f "$DAG_CSV" ]; then
    echo "Converting expert graphs to DAG CSVs..."
    "$PYTHON_BIN" convert_expert_graphs_to_csv.py --dataset cfmnist
fi

echo "Running standalone CREAM on deletion/low/expert3..."
"$PYTHON_BIN" simple_main.py --config all_configs/mcream_configs/cfmnist/cream_noisy_ensemble/cream_noisy_deletion_low_expert3.yaml
echo "Done!"

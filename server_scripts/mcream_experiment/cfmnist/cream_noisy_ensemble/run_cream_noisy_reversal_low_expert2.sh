...
DAG_CSV="$PROJECT_ROOT/data/FashionMNIST/expert_graphs/ensemble/reversal_low/cream_noisy_dags/expert_2.csv"
if [ ! -f "$DAG_CSV" ]; then
    echo "Converting expert graphs to DAG CSVs..."
    "$PYTHON_BIN" convert_expert_graphs_to_csv.py --dataset cfmnist
fi

echo "Running standalone CREAM on reversal/low/expert2..."
"$PYTHON_BIN" simple_main.py --config all_configs/mcream_configs/cfmnist/cream_noisy_ensemble/cream_noisy_reversal_low_expert2.yaml
echo "Done!"

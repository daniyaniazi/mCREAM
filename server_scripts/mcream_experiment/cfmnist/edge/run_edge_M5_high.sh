...
echo "=============================================="
echo "mCREAM: Edge-level Aggregation (M=5, High)"
echo "=============================================="
echo "HOST=$(hostname)"
"$PYTHON_BIN" -V
nvidia-smi || true
...
if [ ! -d "$EXPERT_DIR" ]; then
    echo "Generating expert graphs (high disagreement)..."
    "$PYTHON_BIN" -c "
...
echo ""
echo "Running mCREAM with Edge aggregation (high)..."
"$PYTHON_BIN" mcream_main.py --config all_configs/mcream_configs/cfmnist/edge/edge_M5_high.yaml

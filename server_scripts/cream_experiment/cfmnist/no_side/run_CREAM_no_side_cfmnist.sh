#!/usr/bin/env bash
# CREAM with ground truth DAG, NO side channel — upper bound for no-side comparison
set -euo pipefail
...
echo "Running CREAM no-side-channel (cfmnist)..."
"$PYTHON_BIN" simple_main.py \
    --config all_configs/best_hparams/CREAM_no_side_channel/CREAM_no_side_best_cfmnist_soft_config.yaml
echo "Done!"
